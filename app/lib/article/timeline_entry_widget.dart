...
class VignetteRenderObject extends RenderBox {
    ...
    if (_timelineEntry == null) {
      ...
      if (asset is TimelineNima && asset.actor != null) {
        ...
        if (asset.filename == "assets/Amelia_Earhart/Amelia_Earhart.flr") {
          ...
          _flareController = FilipController();
          _flareController.initialize(_flareActor);
        }
      }
    }
  ...
  flare.Mat2D _viewTransform = flare.Mat2D();
  ...
  void paint(PaintingContext context, Offset offset) {
    ...
    if (asset is TimelineImage) {
      ...
      flare.Mat2D transform = flare.Mat2D();
      Offset globalOffset = this.localToGlobal(renderOffset);
      //print("GL ${globalOffset}");
      transform[4] =
          renderSize.width / 2.0 + (alignment.x * renderSize.width / 2.0);
      transform[5] =
          renderSize.height / 2.0 + (alignment.y * renderSize.height / 2.0);
      flare.Mat2D.scale(
          transform, transform, flare.Vec2D.fromValues(scaleX, scaleY));
      flare.Mat2D center = flare.Mat2D();
      center[4] = x;
      center[5] = y;
