  ...
  void paint(PaintingContext context, Offset offset) {
    ...
    if (heart != null) {
      ...
      for (Heart h in timeline.hearts) {
        ui.Paint heartPaint = ui.Paint()
          ..isAntiAlias = true
          ..filterQuality = ui.FilterQuality.low
          ..color = Colors.white.withOpacity(h.opacity);
        double scale =
            h.opacity * sin(h.phase + h.elapsed * (1.0 + h.phase)).abs();
        double heartSize = (h.end - h.start) * timelineScale * scale;

        double x = offset.dx + timeline.width * h.x - heartSize / 2.0;
        double y = offset.dy +
            (h.start - timeline.renderStart) * timelineScale -
            heartSize / 2.0;
        y += ((y - timelineHeight / 2.0).abs() / timelineHeight) *
            heartParallax *
            h.layer;
